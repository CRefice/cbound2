...
PostProcessStack::PostProcessStack(ssm::ivec2 scrsz) : screen_size(scrsz) {
	...
	ssm::vec2 data[2 * 6] = {
		// Triangle 1
		pos_rect.bottom_left(),  uv_rect.bottom_left(),
		pos_rect.bottom_right(), uv_rect.bottom_right(),
		pos_rect.top_left(),     uv_rect.top_left(),
		// Triangle 2
		pos_rect.top_left(),     uv_rect.top_left(),
		pos_rect.bottom_right(), uv_rect.bottom_right(),
		pos_rect.top_right(),    uv_rect.top_right()
	};

	glBufferData(GL_ARRAY_BUFFER, sizeof(data),
			ssm::data_ptr(data[0]), GL_STATIC_DRAW);

	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE,
			2 * sizeof(ssm::vec2), reinterpret_cast<void*>(0));

	glEnableVertexAttribArray(1);
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE,
			2 * sizeof(ssm::vec2), reinterpret_cast<void*>(sizeof(ssm::vec2)));
