...
using ::render::TileCollisionType;

void load_libraries(sol::state &state) {}

TileCollisionType parse_collision(std::string_view str) {
...
auto parse_tiles(const sol::table &table,
                 Matrix<TileCollisionType> &collisions) {
  ska::flat_hash_map<int, Sequence<int>> tiles;
  tiles.reserve(table.size());
  for (const auto &[key, val] : table) {
    const sol::table &tbl = val;
    auto id = tbl.get<sol::optional<int>>("id");
    if (id) {
      if (auto frames = tbl.get<sol::optional<sol::table>>("frames")) {
        tiles.emplace(*id,
                      Sequence<int>{parse_frames(*frames), PlayMode::Loop});
      }
      if (auto coll = tbl.get<sol::optional<std::string>>("collision_type")) {
        collisions.nth(*id) = parse_collision(*coll);
      }
    } else {
...
std::optional<TileSet> parse_tileset(const sol::table &table) {
  ...
  if (size && tile_size) {
    Matrix<TileCollisionType> collisions(size->x, size->y);
    auto tiles = parse_tiles(table["tiles"], collisions);
