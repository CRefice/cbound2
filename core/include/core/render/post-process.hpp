...
struct PostProcess
{
	PostProcess(Texture&& tex, shader::Program&& prog)
		: texture(std::move(tex)), frame_buffer(texture), program(std::move(prog)) {}
...
{
public:
	PostProcessStack(ssm::ivec2 screen_size);
	...
	void set_screen_size(ssm::ivec2 sz) {
		screen_size = sz;
	}
