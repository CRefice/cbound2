...
public:
  ...
  TextDrawl(std::string text, double sec_per_char = 0.02)
      : txt(std::move(text)), len(0), sec_per_char(sec_per_char),
        countdown(sec_per_char) {}

  auto current_slice() const { return std::string_view(txt.c_str(), len); }

	void set_text(std::string text);

  bool done() const { return len == txt.length(); }

  void progress(double dt);

  void skip() { len = txt.length(); }
